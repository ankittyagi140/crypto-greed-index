...
const CurrentDominance = ({ value, data }: { value: number; data: { date: string; dominance: number }[] }) => {
  ...
  return (
    <div className={`${isPositive ? 'bg-green-100 dark:bg-green-900/40' : 'bg-red-100 dark:bg-red-900/40'} rounded-lg p-4 mb-6 max-w-xs mx-auto border ${isPositive ? 'border-green-200 dark:border-green-800' : 'border-red-200 dark:border-red-800'}`}>
      <div className={`${isPositive ? 'text-green-800 dark:text-green-200' : 'text-red-800 dark:text-red-200'} text-sm font-medium mb-1 text-center`}>Current ETH Dominance</div>
...
export default function ETHDominance({ data, isDetailPage = false }: ETHDominanceProps) {
  ...
  const trendColor = (() => {
    if (filteredData.length < 2) return '#627eea'; // Default ETH blue if not enough data
